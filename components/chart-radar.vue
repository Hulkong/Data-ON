...
export default {
  ...
  methods: {
    ...
    init(options) {
      ...
      const width = Number( d3.select(this.$el).style("width").replace("px", "")) - margin.left - margin.right;
      const height = Number( d3.select(this.$el).style("height").replace("px", "")) - margin.top - margin.bottom;
    ...
    makeAxis(options) {
      ...
      axis
        ...
        .attr("y1", height / 2)
        .attr( "x2",
          (d, i) => ( width / 2 + radius * factor * Math.sin((i * radians) / total))  // x축 전환
        )
        .attr("y2", (d, i) => 
          (height / 2 - radius * factor * Math.cos((i * radians) / total))  // y축 전환
        )
        .attr("class", "line");
