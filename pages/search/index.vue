<template>
  <div id="contents" @click="closeOrder">
    ...
    <div class="u-tab01">
      <ul>
        <li :class="{ active: getCategory == 'title' }">
          <a href="javascript:void(0);" @click="changeCategory('title')">
            <span>데이터 명 ({{ getCount("title_count") }})</span>
          </a>
        </li>
        <li :class="{ active: getCategory == 'detail' }">
          <a href="javascript:void(0);" @click="changeCategory('detail')">
            <span>데이터 설명 ({{ getCount("detail_count") }})</span>
          </a>
        </li>
        <li :class="{ active: getCategory == 'column' }">
          <a href="javascript:void(0);" @click="changeCategory('column')">
            <span>데이터 컬럼 ({{ getCount("column_count") }})</span>
          </a>
        </li>
      </ul>
    </div>
    ...
    <div class="search-wrap">
      <div class="filter">
        ...
        <div class="abr" :class="{ active: order_active }">
          <p>리스트 정렬</p>
          <div class="sel">
            <a href="javascript:void(0);" class="sel-btn" @click="openSelBox">
              <span>{{ getOrderbyNm }}</span>
            </a>
            <ul v-if="orderList.length > 0">
              ...
              >
                <a
                  ...
                  @click="changeOrderby(orderItem.id, orderItem.name)"
                >
        ...
        >
          ...
          <div class="con info">
            ...
            <div
              ...
              v-if="
                docW >= 840 &&
                item.tbresultEvaluation !== undefined &&
                item.tbresultEvaluation !== null
              "
            >
              <chart-radar
                :radarId="item.id"
            ...
            <div class="inups" @click="openDetail(item.id, item.dt_type)">
              ...
              <p
                class="tit"
                :class="{
                  lighton: getCategory == 'title',
                  nolevel: item.dt_level == '원본',
                }"
                v-html="
                  '<strong>' + item.dt_level + '</strong>' + item.dt_title
                "
              ></p>
              ...
              <div class="state" :class="{ mobile: docW < 840 }">
                <dl>
                  <dt v-if="item.dt_type">
                    <span
                      :class="item.dt_type ? item.dt_type.toLowerCase() : ''"
                      >{{ item.dt_type }}</span
                    >
                  </dt>
                  <dd class="c01">
                    <div>
                      용량
                      <span>{{ $setByteUnit(item.dt_byte) }}</span>
                    </div>
                  </dd>
                  <dd class="c02" v-if="docW > 840">
                    <div>
                      전처리 수준
                      <span>{{ item.dt_level }}</span>
                    </div>
                  </dd>
                  <dd class="c03" v-if="docW > 840">
                    <div>
                      최소단위
                      <span>{{ item.dt_range }}</span>
                    </div>
                  </dd>
                  <dd class="c04" v-if="docW > 840">
                    <div>
                      갱신주기
                      <span>{{ item.dt_update || " - " }}</span>
                    </div>
                  </dd>
                  <dd class="c05">
                    <div>
                      다운로드
                      <span>{{ $setComma(item.dt_down) }}건</span>
                    </div>
                  </dd>
                </dl>
              </div>
            ...
            <a
              href="javascript:void(0);"
              @click="getCopyUrl(item.id)"
              class="ex"
              :class="{ off: !detail_id.includes(item.id) }"
              >데이터 다운로드는 PC웹버전에서만 가능합니다.</a
            >
            <!-- 파일 정보 (모바일용2) -->
            ...
            <div class="btns">
              <a
                href="javascript:void(0);"
                class="preview-btn"
                @click="openPopup('previewon', item.id, item.dt_type)"
              >
                <span>미리보기</span>
              </a>
              <a
                v-if="
                  item.tbattachFiles !== undefined &&
                  item.tbattachFiles.length > 0 &&
                  item.dt_type
                "
                href="javascript:void(0);"
                @click="$downloadFile(item.tbattachFiles, downCallback)"
                :class="
                  (item.dt_type ? item.dt_type.toLowerCase() : '') + '-btn'
                "
              >
              ...
              <a
                href="javascript:void(0);"
                class="detail-btn"
                @click="openDetail(item.id, item.dt_type)"
              >
          ...
          <transition name="slide">
            ...
            >
              ...
              <div class="apprs" v-else>
                ...
                <div class="txt">
                  <p style="text-align: center">
                    <img
                      src="/images/Content/img_notify_wait_184h.png"
                      :width="docw > 999 ? '100%' : '184px'"
                    />
                  </p>
                </div>
              </div>
              <!-- e: 평가 -->
              <!-- s: 메타 -->
              <include-detail-meta :info="item"></include-detail-meta>
              <!-- e: 메타 -->
              <!-- s: 컬럼설명 -->
      ...
      <div class="list no-mgt" v-else>
        ...
        <div class="no-item">
          <h3>검색결과가 없습니다.</h3>
          <p class="no-mgt-img"><img src="/images/Content/img_notify_404_03.png" /></p>
          <ul>
      ...
      <!-- s : paging-wrap -->
      <list-page
        v-if="getCount(getCategory + '_count') > 0"
        :page_name="getCategory + '_count'"
        :page_type="name"
      ></list-page>
      <!-- e : paging-wrap -->
      <!-- s : link-list -->
      <slider-other-in
        v-if="isOtherData && this.docW > 999"
        :cnt="searchCnt"
      ></slider-other-in>
      ...
      <speech-bubble
        v-show="getCount(getCategory + '_count') === 0 && isShowSpeechBubble"
        @speech-bubble-close="closeSpeechBubble"
        :style="{ bottom: 0 < this.$parent.$parent.pageY ? '90px' : '33px' }"
      ></speech-bubble>
    </div>
    <!-- e: page-end //-->
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      isShowSpeechBubble: true,   // 검색결과 없을때, 의견남기기 강조 popup on/off
  ...
  async mounted() {
    ...
    if (pasteGbn !== undefined && pasteGbn > -1) {
      ...
      this.pastePage = true;
      // 검색
      await this.setPasteData(pasteGbn);
    } else {
      // 일반 검색인 경우
      if (Object.keys(this.$route.query).length > 1) {
        ...
        this.resetParam(obj);
        if(obj.out === 'succeed') await this.doSearch(this.keyword, 'up', true);
      }else{
        await this.doSearch(this.keyword, 'up');
      }
    }
  ...
  methods: {
    ...
    doSearch: async function (keyword, nonScroll = "up", refreshed) {
      // 로딩 화면 on
     ...
     * 복사한 url 붙여넣기했을때
     */
    setPasteData: async function (id) {
      // 카운터 추가
      await this.addCounts({ post: { col_nm: "dt_paste", id: id } });

      // 데이터 검색
      let posts = _.cloneDeep(this.getPasteOption);
      posts.url = posts.url + id;
      await this.getPasteSearch(posts);
     ...
     */
    closeOrder: function () {
      if (this.order_active && this.contents_click) {
        // 정렬 리스트 open 클릭하고 빈화면 다시 클릭했을때
        this.order_active = false;
        this.contents_click = false;
      } else if (this.order_active) {
        // 정렬 리스트 open 클릭했을때
...
.list >>> .inups p.tit.nolevel strong {
  background: #fff;
  padding: 4px 8px;
  color: #0075c1;
